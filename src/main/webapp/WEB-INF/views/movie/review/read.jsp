...
function printData(moviereview) {
	...
	$("<button></button>").attr("id","btnDelete").attr("class", "btn btn-default").css("margin","10px").text("삭제").appendTo($("#btnArea"));
	// 댓글 출력
	printComment();
...
function printComment(moviereviewcomment) {
	...
	$.each(moviereviewcomment, function(i, comment){
		...
		if(comment.username==loginId){
			$("<textarea id=update_com>").css("width","400px").text(comment.content).prop("readonly",false).appendTo($center_div);
		}else if(comment.username!=loginId){
			$("<span id=update_com>").text(comment.content).prop("readonly",true).appendTo($center_div);
		}
...
function printData(moviereview) {
	...
	if(isLogin==false) {
		...
		$("<button></button>").attr("id","btnDelete").attr("class", "btn btn-default").css("margin","2px").text("삭제").appendTo($("#btnArea"));
		$("<button></button>").attr("id","btnUpdate").attr("class", "btn btn-primary").css("margin","2px").text("변경").appendTo($("#btnArea"));
	} 
	// 댓글 출력
	printComment();
...
function printComment(moviereviewcomment) {
	...
	$.each(moviereviewcomment, function(i, comment){
		...
		$("<span>").text(comment.writingDate).appendTo($lower_div);
		if(comment.username==loginId) {
			var btn = $("<button>").attr("id", "delete_commentt").attr("class", "btn btn-default").attr("data-mrevcmntno", comment.mrevCmntNo)
				.attr("data-mRevNo", moviereview.mRevNo).attr("data-username", comment.username).css("margin","2px").text("삭제").appendTo($center_div);
			btn.css("float","right");
			var btn2 = $("<button>").attr("id", "update_commentt").attr("class", "btn btn-primary").attr("data-mrevcmntno", comment.mrevCmntNo)
				.attr("data-mRevNo", moviereview.mRevNo).attr("data-username", comment.username).css("margin","2px").text("변경").appendTo($center_div);
			btn2.css("float","right");
		}if(comment.username!=loginId){
			var btn3 = $("<button>").attr("id", "like_commentt").attr("class", "btn btn-primary").attr("data-mrevcmntno", comment.mrevCmntNo)
				.attr("data-mRevNo", moviereview.mRevNo).attr("data-username", comment.username).css("margin","2px").text("좋아요"+comment.cmntLikeCnt).appendTo($center_div);
			btn3.css("float","right");
			var btn4 = $("<button>").attr("id", "rep_commentt").attr("class", "btn btn-danger").attr("data-mrevcmntno", comment.mrevCmntNo)
				.attr("data-mRevNo", moviereview.mRevNo).attr("data-username", comment.username).css("margin","2px").text("신고").appendTo($center_div);
...
function printData(moviereview) {
	...
	if(isLogin==false) {
		...
		$("#update_com").prop("readonly", false);
		$("<button></button>").attr("id","btnDelete").attr("class", "btn btn-default").css("margin","2px").text("삭제").appendTo($("#btnArea"));
		$("<button></button>").attr("id","btnUpdate").attr("class", "btn btn-info").css("margin","2px").text("변경").appendTo($("#btnArea"));
	} 
	// 댓글 출력
	printComment();
...
function printComment(moviereviewcomment) {
	...
	$.each(moviereviewcomment, function(i, comment){
		...
		$("<img>").attr("src","/profile/" + comment.username + ".jpg").css("width","40px").appendTo($center_div);
		if(comment.username==loginId){
			$("<textarea id=update_com>")
			.css("width","400px").text(comment.content).prop("readonly",false).appendTo($center_div);
		}else if(comment.username!=loginId){
			$("<span id=update_com>").text(comment.content).prop("readonly",true).appendTo($center_div);
		}
		//$("<span>").text(comment.cmntLikeCnt).appendTo(".like_commentt");
	...
	<div>
		<div id="title_div">
			<input type="hidden" name="mRevNo" id="mRevNo" val="">
				...
				<ul id="lower_right">
					<li>
						<button type="button" class="btn btn-primary" id="a">좋아요 <span class="badge" id="recommend"></span></button>
  						<button type="button" class="btn btn-danger" id="c">신고 <span class="badge" id="report"></span></button> 
  					</li>   
				</ul>
		</div> 
		...
		<div id="content_div">
			...
			<div id="btnArea" style="overflow : hidden">
			</div>
		</div>
	</div>
	<div >
