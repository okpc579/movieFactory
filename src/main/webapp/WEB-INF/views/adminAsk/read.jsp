...
// 답글을 포함한 게시글을 읽어온다
	$(function() {
		...
		var param = {
				adminAskNo : adminAskNo,
			};
		$.ajax({
			url: "/moviefactory/api/adminAsk/answering",
			method: "post",
			data:param,
		...
		$.ajax({
			...
			success : function(result) {
				...
				/*
				if(result==""){ // result가 받아오는 정보값이 없으면 ""으로 적어준다
					location.href="/moviefactory/api/adminAsk/list"
				} 
				*/
				if (adminAsk.askAnswer == null) {
					$("#adminAsk_updateStart").hide();
					$("#adminAsk_delete").hide();
					$("#title").text(adminAsk.title);
					$("#username").text(adminAsk.username);
					$("#adminAskNo").text(adminAsk.adminAskNo);
					$("#askStateContent").text(adminAsk.askStateContent);
